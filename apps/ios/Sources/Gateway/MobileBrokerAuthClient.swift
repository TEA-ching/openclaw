...
public final class MobileBrokerAuthClient: Sendable {
    ...
    private let config: MobileBrokerConfig
    ...
    public func initiateDeviceAuthorization() async throws -> DeviceAuthorizationResponse {
        let url = config.baseURL.appendingPathComponent("v1/device-authorizations")
        ...
        guard (200...299).contains(httpResponse.statusCode) else {
            let error = try? jsonDecoder.decode(BrokerErrorResponse.self, from: data)
    ...
    public func pollDeviceAuthorization(transactionID: String) async throws -> DeviceAuthorizationStatusResponse {
        ...
        if httpResponse.statusCode == 200 {
            return try jsonDecoder.decode(DeviceAuthorizationStatusResponse.self, from: data)
        }
    ...
    public func refreshSession(refreshToken: String) async throws -> RefreshSessionResponse {
        let url = config.baseURL.appendingPathComponent("v1/sessions/refresh")
        ...
        return try jsonDecoder.decode(RefreshSessionResponse.self, from: data)
    }
    ...
    public func revokeCurrentSession(accessToken: String) async throws {
        let url = config.baseURL.appendingPathComponent("v1/sessions/current")
    ...
    ) async throws -> MobileBrokerSessionStore.Session {
        ...
        while Date().timeIntervalSince(startTime) < timeout {
            ...
            case .approved:
                guard let accessToken = statusResponse.accessToken,
                      ...
                      let subjectEmail = statusResponse.subjectEmail else {
                ...
                return MobileBrokerSessionStore.Session(
                    ...
                    brokerHostname: config.hostname
                )
